...
# Li-ion Dataset
li_ion_raw_df = pd.read_csv("D:/Jason/UTSG/Summer Research/Codes for testing/Phase 1 Implementation/li_ion_all_data_combined.csv") # Enter dataset directory
...
# Na-ion Dataset
na_ion_raw_df = pd.read_csv("D:/Jason/UTSG/Summer Research/Dataset/NA-ion/Processed Na Data/na_ion_all_data_combined.csv") # Enter dataset directory
na_df = (na_ion_raw_df.groupby("cell_id", group_keys=False).apply(keep_main_cycling_phase).reset_index(drop=True))
na_df_filtered = filtering_critical_cols(na_df)

# Zn-ion Dataset
zn_ion_raw_df = pd.read_csv("D:/Jason/UTSG/Summer Research/Codes for testing/Phase 1 Implementation/all_data_combined_zn_coin.csv") # Enter dataset directory
...
df_dictionary = {'Li-ion LFP': LFP_df_filtered, 'Li-ion NCA': NCA_df_filtered, 'Li-ion NMC': NMC_df_filtered, 'Na-ion': na_df_filtered, 'Zn-ion': zn_df_filtered}
...
def fit_chemistry(sub_df, chem_name, seed=42):
    ...
    print(f"Median N_80 predicted: {n80_df['N_80_predicted'].median():,.0f}  |  Median actual cycles observed: {n80_df['actual_cycles_observed'].median():,.0f}")

    return {
        'chemistry': chem_name, 'n_cells': len(cells), 'n_train_cells': len(train_cells), 'n_test_cells': len(test_cells),
...
# ---------------------------------------------------------------
colors = {'Li-ion LFP': '#1E2761', 'Li-ion NCA': '#C0392B', 'Li-ion NMC': '#1E8449', 'Na-ion': "#FFE207", 'Zn-ion': "#7F1E84"}
...
axes[0].set_title('Error by Chemistry vs Combined Baseline'); axes[0].legend(fontsize=8); axes[0].grid(True, axis='y', linestyle=':', alpha=0.5)
...
axes[1].set_title('N_80: Predicted vs Actual (per chemistry)'); axes[1].legend(fontsize=8); axes[1].grid(True, axis='y', linestyle=':', alpha=0.5)
...
for chem, res in results.items():
    ...
    axes[2].plot(sample_df['cycle_number'], sample_df['SOH'], marker='o', linestyle='-', color=color, alpha=0.7, linewidth=1, label=f'{chem} actual')
    axes[2].plot(sample_df['cycle_number'], pred, marker='x', linestyle='--', color=color, alpha=0.9, linewidth=1, label=f'{chem} predicted')
...
# ---------------------------------------------------------------
def plot_matched_conditions(results, df_dictionary, colors, temp_tolerance=2.0, rate_tolerance=0.5):
    """
    Create 3 specialized matched-condition plots:
    1. Na-ion vs Zn-ion @ 25°C, 4.5 C-rate
    2. Na-ion vs Zn-ion @ 25°C, 6.0 C-rate
    3. Na-ion vs LFP vs NMC @ 25°C, 2.0 C-rate
    """
    
    # Define target conditions and chemistries for each plot
    plot_specs = [
        {
            'title_prefix': 'Na-ion vs Zn-ion',
            'temp_target': 25.0,
            'rate_target': 4.5,
            'chemistries': ['Na-ion', 'Zn-ion']
        },
        {
            'title_prefix': 'Na-ion vs Zn-ion',
            'temp_target': 25.0,
        ...
        {
            'title_prefix': 'Na-ion vs LFP vs NMC',
            'temp_target': 25.0,
            'rate_target': 2.0,
            'chemistries': ['Na-ion', 'Li-ion LFP', 'Li-ion NMC']
        },
    ]
...
def plot_degradation_rates_matched_bins(results, df_dictionary, colors, temp_tolerance=2.0, rate_tolerance=0.5):
    ...
    plot_specs = [
        ('Na-ion vs Zn-ion', 25.0, 4.5, ['Na-ion', 'Zn-ion']),
        ('Na-ion vs Zn-ion', 25.0, 6.0, ['Na-ion', 'Zn-ion']),
        ('Na-ion vs LFP vs NMC', 25.0, 2.0, ['Na-ion', 'Li-ion LFP', 'Li-ion NMC']),
    ]
    ...
    for panel_idx, (title_prefix, temp_target, rate_target, chems) in enumerate(plot_specs):
        ...
        if not any_chemistry:
            ax_intr.text(0.5, 0.5, 'No chemistry matches this condition', ha='center', va='center', transform=ax_intr.transAxes)
            ax_app.text(0.5, 0.5, 'No chemistry matches this condition', ha='center', va='center', transform=ax_app.transAxes)
        else:
    ...
    fig_app.suptitle('Application-facing Degradation Rate (dSOH/dN, per cycle) versus SOH — matched bins, log scale', fontweight='bold')
